...
## 개요
업비트 KRW 현물 단타. 매시간 2단계 선별(거래대금 top20 → 그 중 등락률 양수 top10)로 후보를 잡고,
5분봉 마감마다 단기 추세가 상승인 종목만 SMC(Smart Money Concept) 관점으로 진입을 판단한다. 티어/승강 구조 없음.

## 파이프라인 (4 독립 서비스, DB로만 소통)
1. scanner — 매시간 거래대금(24h) top20 → 그 중 등락률 양수 top10 → scan_candidates
...
- 유효 long 은 **구조적 근거**가 있을 때만:
  (A) **돌파-리테스트**: 저항을 이미 돌파(brk_hi)한 뒤 그 레벨로 **되돌아와(former resistance→support) 지지받을 때** 거래량 동반 진입. **초기 돌파 캔들 추격 금지** — 리테스트가 진짜 타점.
